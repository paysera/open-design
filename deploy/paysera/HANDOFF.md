...
- **Live artifact:** `GET /api/live-artifacts/:id/preview` and `POST .../refresh` are loopback-only upstream (desktop-app design); behind our proxy the daemon never sees a loopback peer/host/origin, so they 403'd. Relaxed in `routes/live-artifact.ts` to pass when `OD_DISABLE_API_AUTH=1` (trusted proxy). Daemon shutdown / db vacuum stay strict — verified still 403 from a non-loopback peer.
- **Connectors / Integrations:** the `/api/connectors/*` setup endpoints were loopback-only too → relaxed in `connectors/routes.ts` under `OD_DISABLE_API_AUTH=1`. Needed for the Integrations tab and connector-backed live-artifact refresh sources.
- **Live-artifact refresh:** only **data-backed** artifacts (those with a registered `sourceJson`) can refresh. A static UI artifact (e.g. the wallet mockup) reports "No refresh source is available yet" — that's correct, not a bug. Refreshable sources that work behind the proxy: `daemon_tool` (project files, git summary, public GitHub repo metric — run server-side) and now Composio connectors. Pointing an artifact at its own `data.json` is not a valid source (circular) — the tooling rejects it.
- **Media (image / video / voice):** separate provider layer (`media-config.ts`), keyed per provider via env (`ENV_KEYS`, env wins) or the Settings UI; the base URL comes ONLY from `<OD_DATA_DIR>/media-config.json`, not env. Wired through the litellm gateway: the `openai` media provider's `baseUrl` is set to the gateway in `media-config.json` (startup writes it idempotently) + key from `OPENAI_API_KEY` (the litellm virtual key). So **image (gpt-image-2) and voice/TTS (gpt-4o-mini-tts via `/v1/audio/speech`, verified 200) go through LiteLLM** with one key. **Video does NOT go through LiteLLM** — the gateway serves no video models and OD has no litellm-video adapter. For video use the `google` provider (Veo, via the Gemini key — verify Veo access on the key) or a dedicated provider (Kling, Seedance/volcengine, Grok, fal, OpenRouter), each its own key.
